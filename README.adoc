...
* usage of `useSWR` even for data that is already pre-rendered. Presumably this is to give some kind of pool based realtime support? But that is not what other implementations do, and neither should we. We don't want data to update by surprise under a user's feet.
