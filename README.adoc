...
This is done because this repository is normally developped as a branch of that one, which would lead to a conflicting name for the branch `heroku`.

You then have to add `--app cirosantilli-realworld-next` to allow heroku to differentiate between them, e.g.:
