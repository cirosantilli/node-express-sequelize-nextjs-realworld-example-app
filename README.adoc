...
The `--` is important to allow further `--` arguments to be passed correctly, e.g. `--force-production` requires it if you want to delete, recreate and reseed the database:

....
npm run heroku -- bin/generate-demo-data.js --force-production
....
